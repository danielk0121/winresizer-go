...
- **사이즈 리사이클**: 단축키를 반복 입력하면 창 폭이 10%씩 증감 (최소 20% ↔ 최대 90% 랩어라운드)
- **커스텀 비율 조절**: 좌/우/상/하 방향별로 원하는 비율(1~100%)을 직접 지정하여 단축키 등록
...
- **중복 실행 방지**: 단축키 연속 입력 시 이전 명령 완료 전 추가 호출 자동 드롭 (atomic debounce)
...
![앱_스크린샷](ref/img_7.png)

## 설치 및 실행 (Quick Guide)

```bash
# 1. 소스코드 복사
git clone https://github.com/danielk0121/winresizer-go.git
...
- **GitHub 릴리즈 배포**: `build-work/release.sh v1.0.0` 형식으로 DMG를 GitHub Release에 업로드할 수 있습니다.
...
│   └── utils/              # 로거 및 공통 유틸리티
├── build-work/             # .app 번들 빌드, 아이콘 생성, 코드 서명, GitHub 릴리즈 스크립트
...
`default-config.json`에 정의된 기본 단축키입니다. 설정 UI에서 자유롭게 변경할 수 있습니다.
...
| 하단 1/2 | `ctrl + opt + ↓` |
| 좌측 확장 (+10%) | `ctrl + opt + ←` |
| 좌측 축소 (-10%) | `ctrl + opt + shift + ←` |
| 우측 확장 (+10%) | `ctrl + opt + →` |
| 우측 축소 (-10%) | `ctrl + opt + shift + →` |
| 커스텀 비율 좌측 | 기본값 60%, 단축키 미설정 |
| 커스텀 비율 우측 | 기본값 60%, 단축키 미설정 |
| 커스텀 비율 상단 | 기본값 70%, 단축키 미설정 |
| 커스텀 비율 하단 | 기본값 70%, 단축키 미설정 |
| 좌측 1/2, 우측 1/2 등 | 미설정 (UI에서 직접 등록) |
...
[Go Callback Handler] (core/hotkey_listener_darwin.go)
   ↓ (명령어 매칭)
[ExecuteWindowCommand] (core/window_controller.go)
   ↓ (atomic debounce — 이전 명령 처리 중이면 드롭)
   ├─ 일반 모드 → [좌표 계산] → [SetWindowFrame] → [re-anchor 보정]
   ├─ 스마트 사이클 → isAlreadyAligned 판단 → nextCycleMode 재귀 호출
   └─ 사이즈 리사이클 → 현재 폭 ±10% 계산 (20%↔90% 랩어라운드) → [SetWindowFrame]
